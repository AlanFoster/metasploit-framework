...
module ReportGeneration
  ...
  def self.extract_data(options)
    ...
    if options[:allure_data]
      puts 'extracting data inside allure data'
      results_directory = options[:allure_data]
      puts "results directory: #{results_directory}"

      puts "ls -la #{results_directory}"
      puts `ls -la #{results_directory}`

      puts "find #{results_directory}"
      puts `find #{results_directory}`

      test_result_files = Dir["**/*-result.json", base: results_directory]
      puts "Finding result files #{test_result_files}"
