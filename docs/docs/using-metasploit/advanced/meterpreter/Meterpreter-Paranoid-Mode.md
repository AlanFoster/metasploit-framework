...
For this use case, we will combine [Payload UUID]({% link docs/using-metasploit/intermediate/Payload-UUID.md %}) tracking and whitelisting with [TLS pinning]({% link docs/using-metasploit/advanced/meterpreter/Meterpreter-HTTP-Communication.md %}#tls-certificate-pinning). For a staged payload, we will use the following command:
...
A [stageless]({% link docs/using-metasploit/advanced/meterpreter/Meterpreter-Stageless-Mode.md %}) version of this would look like the following:
