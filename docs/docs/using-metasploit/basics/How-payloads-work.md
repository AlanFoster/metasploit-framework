...
1. The Ruby side acts as a client using whichever transport mechanism was set up by the stager (e.g.: tcp, http, https).
   ...
   * In the case of a [Meterpreter]({% link docs/using-metasploit/advanced/meterpreter/Meterpreter.md %}) stage, Metasploit will begin speaking the Meterpreter wire protocol.
