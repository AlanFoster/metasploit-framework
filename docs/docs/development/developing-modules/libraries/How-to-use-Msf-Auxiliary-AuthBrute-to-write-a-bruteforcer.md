...
The ```Msf::Auxiliary::AuthBrute``` mixin should no longer be used to write a login module, you should try our [LoginScanner API]({% link docs/development/developing-modules/guides/scanners/Creating-Metasploit-Framework-LoginScanners.md %}) instead. However, some of the datastore options are still needed, so let's go over them right quick.
