...
We do that in execute_command using [HttpClient]({% link docs/development/developing-modules/libraries/http/How-to-Send-an-HTTP-Request-Using-HttpClient.md %}). Notice there is actually some bad character filtering involved to get the exploit working correctly, which is expected:
...
If you are exploiting Windows that supports Powershell, then you might want to [consider using that instead]({% link docs/development/developing-modules/libraries/How-to-use-Powershell-in-an-exploit.md %}) of the VBS stager, because Powershell tends to be more stealthy.
