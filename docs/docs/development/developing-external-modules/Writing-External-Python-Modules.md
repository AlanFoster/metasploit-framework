...
The `remote_exploit_cmd_stager` module type is used when writing an exploit for command execution or code injection vulnerabilities and provides the command to inject into the vulnerable code based on the [flavor]({% link docs/development/developing-modules/How-to-use-command-stagers.md %}) specified for the command stager.
