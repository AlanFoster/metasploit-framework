...
def self.generate_command_id_map_rust
  id_map = [
    COMMAND_ID_MAP_CORE,
    COMMAND_ID_MAP_STDAPI,
  ].inject({}) {|m1, m2| m1.merge(m2)}
  command_ids = id_map.map { |k, v| "            (#{v}, \"#{k}\")," }
  %Q^
// ---------------------------------------------------------------
// --- THIS CONTENT WAS GENERATED BY A TOOL @ #{Time.now.utc}
// ---------------------------------------------------------------
...
#[rustfmt::skip]
lazy_static! {
    static ref COMMAND_ID_MAP: HashMap<u32, &'static str> = {
        HashMap::from([
#{command_ids.join("\n")}
        ])
    };
    static ref COMMAND_NAME_MAP: HashMap<&'static &'static str, &'static u32> = {
        let mut map = HashMap::new();
        for (id, str) in COMMAND_ID_MAP.iter() {
            map.insert(str, id);
        }
        map
    };
...
fn command_id_to_str(id: u32) -> Option<String> {
    COMMAND_ID_MAP
        .get(&id)
        .map(|command_str| command_str.to_string())
}

fn command_str_to_id(str: &str) -> Option<u32> {
    COMMAND_NAME_MAP.get(&str).map(|id| **id)
}
// ---------------------------------------------------------------
  ^
end
